...
main(int argc, char **argv)
{
	...
	if (argc > 1){
		ttl_arg = strtol(argv[1], NULL, 10);
	}
	if (argc > 2){
		hops_arg = strtol(argv[2], NULL, 10);
	}
	if ( (sockfd = socket(AF_INET6, SOCK_DGRAM, 0)) < 0){
			fprintf(stderr,"socket error : %s\n", strerror(errno));
			return 1;
	}
	if (setsockopt(sockfd, IPPROTO_IP, IP_TTL, &ttl_arg, sizeof(ttl_arg)) < 0){
	...
	if (setsockopt(sockfd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &hops_arg, sizeof(hops_arg)) < 0){
		fprintf(stderr, "IPV6_UNICAST_HOPS error\n");
	}
	else{
		printf("UNICAST_HOPS: %d\n", hops_arg);
	}
