...
int dt_cli(int sockfd, const SA *pservaddr, socklen_t servlen, struct sockaddr	*preply_addr, 
...
//	ssize_t			t;
	...
	union {
	  struct cmsghdr	cm;
	  char				control[CMSG_SPACE(16+sizeof(struct in6_pktinfo))];
	} control_un;
	...
	if(setsockopt(sockfd, IPPROTO_IP, IP_RECVTTL, &ttl, sizeof(ttl))<0){
		fprintf(stderr, "IP_RECVTTL error\n");
	}
...
//	}
	...
	*data_len = n;
	...
	for (cmptr = CMSG_FIRSTHDR(&msg); cmptr != NULL;
		...
		if(setsockopt(sockfd, IPPROTO_IP, IP_TTL, &received_ttl, sizeof(received_ttl))<0){
			fprintf(stderr, "IP TTL ERROR\n");
		}else{
			printf("TTL: %d\n", received_ttl);
		}
